 ...
 */
package org.aves.transfer.json;
...
public class JSONObject implements Serializable {
	...
	public JSONObject(Map map) {
		this.myHashMap = (map == null) ? new LinkedHashMap()
	...
	protected JSONObject doPut(String key, Object value, int threshold,
			boolean checkExistingValue) throws JSONException {
		if (key == null) {
		...
		if (value != null) {
			...
			if (!checkExistingValue || !this.myHashMap.containsKey(key)) {
				...
				if (threshold > 0 && myHashMap.size() >= threshold) {
					throw new JSONSequenceTooLargeException(
				...
				if (existingValue instanceof JSONArray) {
					...
					array = new JSONArray(
							Collections.singletonList(existingValue));
