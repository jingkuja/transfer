...
public class ExportExcel {
	...
	public void processExcel(
			Map<String, List<List<Map<String, String>>>> excem,
			OutputStream out, boolean sacle, boolean ver, boolean title)
			throws FileNotFoundException, IOException {
		try {
			HSSFWorkbook wbd = new HSSFWorkbook();
			int v = 0;
			for (Map.Entry<String, List<List<Map<String, String>>>> entryd : excem
					.entrySet()) {
				String sheetname = entryd.getKey();
				List<List<Map<String, String>>> cm = entryd.getValue();
				outDatasInSheet(wbd, cm, sheetname, sacle, ver, title, v);
				v++;
			}
			wbd.write(out);
			if (out != null) {
	...
	private void outDatasInSheet(HSSFWorkbook wb,
			List<List<Map<String, String>>> cm, String sheetname,
			boolean sacle, boolean ver, boolean title, int sheetpo)
			throws FileNotFoundException, IOException {
		HSSFSheet sheet = wb.createSheet(); // 产生工作表对象
		if (title)
			sheet.addMergedRegion(new Region(0, (short) 3, 0, (short) 8));
		...
		// 设置左右
		cellStyle.setVerticalAlignment(CellStyle.ALIGN_CENTER);
		wb.setSheetName(sheetpo, sheetname);
		for (int i = 0; i < cm.size(); i++) {
			if (i == 0) {
				cellStyle.setFont(fontd);
			} else {
